...
public class SolrPrivilegeValidator implements PrivilegeValidator {
  ...
  private void extraPrivilegeValidation(String entityType, String entityName, String actionName) {
    if ("admin".equals(entityType)) {
      if (!Admin.ENTITY_NAMES.contains(entityName)) {
        throw new IllegalArgumentException(
            "Invalid entity name specified for the admin entity type. Valid names are " + Admin.ENTITY_NAMES);
      } else if (Admin.METRICS.getName().equals(entityName) && !SolrConstants.QUERY.equals(actionName)) {
        throw new IllegalArgumentException(
