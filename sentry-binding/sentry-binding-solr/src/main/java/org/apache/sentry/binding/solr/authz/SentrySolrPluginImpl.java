...
import org.apache.sentry.core.model.solr.Admin;
...
public class SentrySolrPluginImpl implements AuthorizationPlugin {
  ...
  public AuthorizationResponse authorize(AuthorizationContext authCtx) {
      ...
      switch (perm) {
        ...
        case UPDATE_PERM: {
          ...
          for (CollectionRequest req : authCtx.getCollectionRequests()) {
            ...
            if (resp != AuthorizationResponse.OK) {
              return audit (perm, authCtx, resp);
            }
          }
          return audit (perm, authCtx, AuthorizationResponse.OK);
        }
        case SECURITY_EDIT_PERM: {
          return binding.authorize(userName, Collections.singleton(Admin.SECURITY), UPDATE);
        }
        case SECURITY_READ_PERM: {
          return binding.authorize(userName, Collections.singleton(Admin.SECURITY), QUERY);
        }
        case CORE_READ_PERM:
        case CORE_EDIT_PERM:
        case COLL_READ_PERM:
        case COLL_EDIT_PERM: {
          SolrModelAuthorizable auth = (perm == Name.COLL_READ_PERM || perm == Name.COLL_EDIT_PERM)
              ? Admin.COLLECTIONS : Admin.CORES;
          Set<SolrModelAction> actions = (perm == Name.COLL_READ_PERM || perm == Name.CORE_READ_PERM)
              ? QUERY : UPDATE;
          AuthorizationResponse r = audit (perm, authCtx, binding.authorize(
              userName, Collections.singleton(auth), actions));
          if (AuthorizationResponse.OK.equals(r)) {
            ...
            for (Map.Entry<String, SolrModelAction> entry :
              ...
              AuthorizationResponse t = audit(p, authCtx, binding.authorizeCollection(userName,
                  new Collection(entry.getKey()), Collections.singleton(entry.getValue())));
              if (AuthorizationResponse.FORBIDDEN.equals(t)) {
                return t;
              }
            }
          }
        ...
        case METRICS_READ_PERM: {
          return binding.authorize(userName, Collections.singleton(Admin.METRICS), QUERY);
        }
        case AUTOSCALING_READ_PERM:
        case AUTOSCALING_HISTORY_READ_PERM: {
          return binding.authorize(userName, Collections.singleton(Admin.AUTOSCALING), QUERY);
        }
        case AUTOSCALING_WRITE_PERM: {
          return binding.authorize(userName, Collections.singleton(Admin.AUTOSCALING), UPDATE);
        }
        case ALL: {
          return AuthorizationResponse.OK;
        }
    ...
    // This handler does not provide authorization meta-data. Allow this handler to proceed.
    return AuthorizationResponse.OK;
  }
  ...
  private AuthorizationResponse audit (Name perm, AuthorizationContext ctx, AuthorizationResponse resp) {
